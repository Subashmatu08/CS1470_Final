...
parser = argparse.ArgumentParser(description='TensorFlow Evaluation')
...
train_set_expanded = tf.expand_dims(train_set, axis=0) 
test_set_expanded = tf.expand_dims(test_set, axis=1)

distances_matrix = tf.reduce_sum(tf.square(test_set_expanded - train_set_expanded), axis=2)
distances = tf.reduce_min(distances_matrix, axis=1).numpy()
...
print("\nDistance statistics:")
print(f"  Mean real distances: {np.mean(distances[labels==1]):.2f} ± {np.std(distances[labels==1]):.2f}")
print(f"  Mean fake distances: {np.mean(distances[labels==0]):.2f} ± {np.std(distances[labels==0]):.2f}")
