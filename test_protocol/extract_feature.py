...
if __name__ == '__main__':
    ...
    path_names = []
    for root, _, f_names in tqdm(os.walk(args.input_root)):
        cur_vid_embeddings = []
        for frame in f_names:
            if not frame.lower().endswith(('.jpg', '.jpeg', '.png')):  # Support multiple image formats
                continue
            p = os.path.join(root, frame)  # Use full path for processing
            path_names.append(p)
            image = cv2.imdecode(np.fromfile(p, dtype=np.uint8), cv2.IMREAD_UNCHANGED)
